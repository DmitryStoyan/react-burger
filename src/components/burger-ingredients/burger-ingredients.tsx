...
function BurgerIngredients() {
  ...
  const BUNS = ingredientsArray.filter(
    (ingredient) => ingredient.type === ProductType.Bun.type
  );
  const MAINS = ingredientsArray.filter(
    (ingredient) => ingredient.type === ProductType.Main.type
  );
  const SAUCES = ingredientsArray.filter(
    (ingredient) => ingredient.type === ProductType.Sauce.type
  );

  return (
    <section
      className={`${styles.products} pt-10`}
      aria-label={ariaLable.ingridients}
    >
      ...
      />
      <div className={`${styles.listsWrapper} pt-10 burgerIngredientsScroll`}>
        <ProductList items={BUNS} itemsType={ProductType.Bun} ref={bunsRef} />
        <ProductList
          items={SAUCES}
          itemsType={ProductType.Sauce}
          ref={saucesRef}
        />
        <ProductList
          items={MAINS}
          itemsType={ProductType.Main}
          ref={mainsRef}
        />
      </div>
    </section>
